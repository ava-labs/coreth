...
func TestFeeInfoProvider(t *testing.T) {
	backend := newTestBackend(t, extparams.TestChainConfig, 2, common.Big0, testGenBlock(t, 55, 370))
...
func TestFeeInfoProviderCacheSize(t *testing.T) {
	size := 5
	overflow := 3
	backend := newTestBackend(t, extparams.TestChainConfig, 0, common.Big0, testGenBlock(t, 55, 370))
