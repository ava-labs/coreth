...
type UpgradeConfig = params.UpgradeConfig
...
func SetEthUpgrades(c *ChainConfig) {
	if c.ChainID != nil && params.AvalancheFujiChainID.Cmp(c.ChainID) == 0 {
	...
	} else if c.ChainID != nil && params.AvalancheMainnetChainID.Cmp(c.ChainID) == 0 {
...
func GetChainConfig(agoUpgrade upgrade.Config, chainID *big.Int) *ChainConfig {
	return WithExtra(
		...
		&params.ChainConfigExtra{NetworkUpgrades: params.GetNetworkUpgrades(agoUpgrade)},
	)
}
