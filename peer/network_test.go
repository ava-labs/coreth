...
func TestNetworkDoesNotConnectToItself(t *testing.T) {
	...
	n := NewNetwork(p2p.NewRouter(), nil, nil, nil, selfNodeID, 1, 1)
...
func TestRequestAnyRequestsRoutingAndResponse(t *testing.T) {
	...
	net = NewNetwork(p2p.NewRouter(), sender, codecManager, crossChainCodecManager, ids.EmptyNodeID, 16, 16)
...
func TestAppRequestOnShutdown(t *testing.T) {
	...
	codecManager := buildCodec(t, HelloRequest{}, HelloResponse{})
	crossChainCodecManager := buildCodec(t, ExampleCrossChainRequest{}, ExampleCrossChainResponse{})
	net = NewNetwork(p2p.NewRouter(), sender, codecManager, crossChainCodecManager, ids.EmptyNodeID, 1, 1)
	client := NewNetworkClient(net)
	nodeID := ids.GenerateTestNodeID()
...
func TestRequestMinVersion(t *testing.T) {
	...
	net = NewNetwork(p2p.NewRouter(), sender, codecManager, crossChainCodecManager, ids.EmptyNodeID, 1, 16)
	client := NewNetworkClient(net)
...
func TestGossip(t *testing.T) {
	...
	clientNetwork = NewNetwork(p2p.NewRouter(), sender, codecManager, crossChainCodecManager, ids.EmptyNodeID, 1, 1)
