...
func (t *trieToSync) loadSegments() error {
	...
	it := crawdb.NewSyncSegmentsIterator(t.sync.db, t.root)
	defer it.Release()
	...
	for it.Next() {
		...
		// the first segment.
		_, segmentStart := crawdb.UnpackSyncSegmentKey(it.Key())
...
func (t *trieToSync) segmentFinished(ctx context.Context, idx int) error {
	...
	if err := crawdb.ClearSyncSegments(t.sync.db, t.root); err != nil {
		return err
	}
...
func (t *trieToSync) createSegments(numSegments int) error {
	...
	for i := 0; i < numSegments; i++ {
		...
		if err := crawdb.WriteSyncSegment(t.sync.db, t.root, common.BytesToHash(segment.start)); err != nil {
			return err
		}
	}
