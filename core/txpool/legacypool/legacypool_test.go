...
func dynamicFeeTx(nonce uint64, gaslimit uint64, gasFee *big.Int, tip *big.Int, key *ecdsa.PrivateKey) *types.Transaction {
	tx, _ := types.SignNewTx(key, types.LatestSignerForChainID(extparams.TestChainConfig.ChainID), &types.DynamicFeeTx{
...
func setupPool() (*LegacyPool, *ecdsa.PrivateKey) {
	return setupPoolWithConfig(extparams.TestChainConfig)
}
...
func TestStateChangeDuringReset(t *testing.T) {
	...
	blockchain := &testChain{newTestBlockChain(extparams.TestChainConfig, 1000000000, statedb, new(event.Feed)), address, &trigger}
...
func TestPostponing(t *testing.T) {
	...
	statedb, _ := state.New(types.EmptyRootHash, state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
	blockchain := newTestBlockChain(extparams.TestChainConfig, 1000000, statedb, new(event.Feed))
