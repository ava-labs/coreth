...
func opReturnDataCopy(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	if overflow {
		return nil, coreerrors.ErrReturnDataOutOfBounds
	}
...
func opJump(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	if !scope.Contract.validJumpdest(&pos) {
		return nil, coreerrors.ErrInvalidJump
	}
...
func opCreate(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	if interpreter.evm.chainRules.IsHomestead && suberr == coreerrors.ErrCodeStoreOutOfGas {
		stackvalue.Clear()
	} else if suberr != nil && suberr != coreerrors.ErrCodeStoreOutOfGas {
		stackvalue.Clear()
	} else {
	...
	if suberr == coreerrors.ErrExecutionReverted {
...
func opCall(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	stack.push(&temp)
	if err == nil || err == coreerrors.ErrExecutionReverted {
...
func opRevert(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	interpreter.returnData = ret
	return ret, coreerrors.ErrExecutionReverted
}
