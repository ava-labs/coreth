...
func TestNetworkUpgradeBetweenHeadAndAcceptedBlock(t *testing.T) {
	...
	config, _, err := setupGenesisBlock(db, trie.NewDatabase(db, trie.HashDefaults), &activatedGenesis, bc.lastAccepted.Hash())
	require.NoError(err)
