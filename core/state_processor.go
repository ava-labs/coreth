...
func ApplyPrecompileActivations(c *params.ChainConfig, parentTimestamp *uint64, blockContext contract.ConfigurationBlockContext, statedb *state.StateDB) error {
	...
	for _, module := range modules.RegisteredModules() {
		for _, activatingConfig := range c.GetActivatingPrecompileConfigs(module.Address, parentTimestamp, blockContext.Time(), c.PrecompileUpgrades) {
...
func (t timestampToTime) Time() uint64 {
	return t.BlockGen.Timestamp()
}
