	...
	for i := start; i <= stop; i++ {
		...
		if tapeRecorder != nil {
			...
			if i%uint64(logEach) == 0 {
				...
				lastLogTime = time.Now()
				t.Logf(
					"(%v) Block: %d, Txs: %d (+ %d atomic), db root: %s, block root: %s",
					took.Truncate(time.Millisecond), i, len(block.Transactions()), len(atomicTxs),
					lastInsertedRoot.TerminalString(), block.Root().TerminalString())
			}
		}
