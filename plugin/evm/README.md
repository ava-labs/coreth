...
To do this, the VM uses a modified version of the Ethereum RLP block type in [block.go](../../core/types/block.go) and uses the core package's BlockChain type in [blockchain.go](../../core/blockchain.go) to handle the insertion and storage of blocks into the chain.
...
The Block type wraps the stateless block type in [block.go](../../core/types/block.go) and implements these functions to allow the consensus engine to verify blocks as valid, perform consensus, and mark them as accepted or rejected. See the documentation in AvalancheGo for the more detailed VM invariants that are maintained here.
