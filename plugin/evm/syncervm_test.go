...
func TestStateSyncToggleEnabledToDisabled(t *testing.T) {
	...
	appSender.SendAppRequestF = func(_ context.Context, nodeSet ids.NodeIDSet, requestID uint32, request []byte) error {
		nodeID, hasItem := nodeSet.Pop()
		if !hasItem {
			t.Fatal("expected nodeSet to contain at least 1 nodeID")
		}
		go vmSetup.serverVM.AppRequest(context.Background(), nodeID, requestID, time.Now().Add(1*time.Second), request)
		return nil
	}
	...
	vmSetup.serverAppSender.SendAppResponseF = func(_ context.Context, nodeID ids.NodeID, requestID uint32, response []byte) error {
		if test.responseIntercept == nil {
			go syncReEnabledVM.AppResponse(context.Background(), nodeID, requestID, response)
		} else {
...
func createSyncServerAndClientVMs(t *testing.T, test syncTest) *syncVMSetup {
	...
	syncerAppSender.SendAppRequestF = func(_ context.Context, nodeSet ids.NodeIDSet, requestID uint32, request []byte) error {
		nodeID, hasItem := nodeSet.Pop()
		if !hasItem {
			t.Fatal("expected nodeSet to contain at least 1 nodeID")
		}
		go serverVM.AppRequest(context.Background(), nodeID, requestID, time.Now().Add(1*time.Second), request)
		return nil
	}
