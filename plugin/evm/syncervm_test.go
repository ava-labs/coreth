...
func TestSkipStateSync(t *testing.T) {
	...
	vmSetup := createSyncServerAndClientVMs(t, test, vmsync.ParentsToFetch)
...
func TestStateSyncToggleEnabledToDisabled(t *testing.T) {
	...
	syncDisabledVM := newDefaultTestVM()
	...
	syncReEnabledVM := newDefaultTestVM()
...
func createSyncServerAndClientVMs(t *testing.T, test syncTest, numBlocks int) *syncVMSetup {
	...
	serverAtomicTrie := serverVM.atomicBackend.AtomicTrie()
