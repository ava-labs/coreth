...
func TestSubscribeToEvents(t *testing.T) {
	...
	t.Run("SubscribeToEvents with context cancelled returns 0", func(t *testing.T) {
		...
		go func() {
			defer wg.Done()
			require.Equal(t, commonEng.Message(0), tvm.vm.SubscribeToEvents(ctx))
		}()

		wg.Wait()
	})

	t.Run("SubscribeToEvents returns when a transaction is added to the mempool", func(t *testing.T) {
		importTx, err := tvm.vm.newImportTx(tvm.vm.ctx.XChainID, address, initialBaseFee, []*secp256k1.PrivateKey{testKeys[0]})
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("SubscribeToEvents doesn't return once a block is build and accepted", func(t *testing.T) {
		blk, err := tvm.vm.BuildBlock(context.Background())
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("SubscribeToEvents returns when regular transactions are added to the mempool", func(t *testing.T) {
		txs := make([]*types.Transaction, 10)
		for i := 0; i < 10; i++ {
			tx := types.NewTransaction(uint64(i), address, big.NewInt(10), 21000, big.NewInt(3*ap0.MinGasPrice), nil)
			signedTx, err := types.SignTx(tx, types.NewEIP155Signer(tvm.vm.chainID), key)
	...
	t.Run("SubscribeToEvents returns 0 when VM is shutdown", func(t *testing.T) {
		ctx, cancel := context.WithTimeout(context.Background(), time.Millisecond*100)
		defer cancel()
