...
func TestSubscribeToEvents(t *testing.T) {
	...
	assertNoSubscriptionEventsOutputted(t, vm)
...
func assertNoSubscriptionEventsOutputted(t *testing.T, vm *VM) {
	...
	require.False(t, time.Since(subscriptionTime) < time.Millisecond*10)
}
...
func TestSkipChainConfigCheckCompatible(t *testing.T) {
	...
	err = reinitVM.Initialize(context.Background(), vm.ctx, dbManager, genesisWithUpgradeBytes, []byte{}, []byte{}, nil, []*commonEng.Fx{}, appSender)
	...
	err = reinitVM.Initialize(context.Background(), vm.ctx, dbManager, genesisWithUpgradeBytes, []byte{}, config, nil, []*commonEng.Fx{}, appSender)
	require.NoError(t, err)
