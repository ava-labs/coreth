...
func (vm *VM) newChainDB(db avalanchedatabase.Database) (ethdb.Database, error) {
	...
	if database.IsBlockDBInitialized(blockdbStateDB) && !vm.config.BlockDatabaseEnabled {
	...
	// wrap chaindb if dedicated block database is enabled
	if vm.config.BlockDatabaseEnabled {
		...
		autoMigrate := !vm.config.BlockDatabaseMigrationDisabled
		chainDB, err = database.NewWrappedEthDatabase(db, blockDBPath, chainDB, stateSyncEnabled, config, autoMigrate, vm.ctx.Log, vm.sdkMetrics)
		if err != nil {
			return nil, fmt.Errorf("failed to wrap block database: %w", err)
		}
	}
	return chainDB, nil
}
