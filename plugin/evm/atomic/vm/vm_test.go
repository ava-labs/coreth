...
func TestImportMissingUTXOs(t *testing.T) {
	...
	require.NoError(t, vm1.mempool.AddLocalTx(importTx))
	<-tvm1.ToEngine
	...
	badBlocks, _ := vm2.InnerVM.Ethereum().BlockChain().BadBlocks()
...
func TestIssueAtomicTxs(t *testing.T) {
	...
	vm.InnerVM.Ethereum().BlockChain().DrainAcceptorQueue()
...
func testConflictingImportTxs(t *testing.T, fork upgradetest.Fork) {
	...
	vm, tvm := setupAtomicTestVMWithUtxos(t, vmtest.TestVMConfig{
		Fork: &fork,
	}, map[ids.ShortID]uint64{
		vmtest.TestShortIDAddrs[0]: importAmount,
...
func TestBuildApricotPhase5Block(t *testing.T) {
	...
	ethBlkTxs := vm.InnerVM.Ethereum().BlockChain().GetBlockByNumber(2).Transactions()
	for i, tx := range txs {
		if len(ethBlkTxs) <= i {
...
func TestBuildInvalidBlockHead(t *testing.T) {
	...
	currentBlock := vm.InnerVM.Ethereum().BlockChain().CurrentBlock()
	...
	newCurrentBlock := vm.InnerVM.Ethereum().BlockChain().CurrentBlock()
