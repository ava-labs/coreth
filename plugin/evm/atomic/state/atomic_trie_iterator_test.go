...
func TestIteratorCanIterate(t *testing.T) {
	...
	atomicBackend, err := NewAtomicBackend(atomictest2.TestSharedMemory(), nil, repo, lastAcceptedHeight, common.Hash{}, 100)
	assert.NoError(t, err)
	...
	verifyOperations(t, atomicTrie1, atomictest2.TestTxCodec, lastCommittedHash1, 1, 1000, operationsMap)

	// iterate on a new atomic trie to make sure there is no resident state affecting the data and the
	// iterator
	atomicBackend2, err := NewAtomicBackend(atomictest2.TestSharedMemory(), nil, repo, lastAcceptedHeight, common.Hash{}, 100)
	assert.NoError(t, err)
	...
	verifyOperations(t, atomicTrie2, atomictest2.TestTxCodec, lastCommittedHash1, 1, 1000, operationsMap)
}
...
func TestIteratorHandlesInvalidData(t *testing.T) {
	...
	atomicBackend, err := NewAtomicBackend(atomictest2.TestSharedMemory(), nil, repo, lastAcceptedHeight, common.Hash{}, commitInterval)
	require.NoError(err)
	...
	verifyOperations(t, atomicTrie, atomictest2.TestTxCodec, lastCommittedHash, 1, 1000, operationsMap)
