...
func TestSendWarpMessage(t *testing.T) {
	...
	vm, _, _, _ := GenesisVM(t, true, genesisJSONDurango, "", "")
	...
	require.Equal(commonEng.PendingTxs, vm.SubscribeToEvents(context.Background()))
...
func TestMessageSignatureRequestsToVM(t *testing.T) {
	vm, _, _, appSender := GenesisVM(t, true, genesisJSONDurango, "", "")
...
func TestClearWarpDB(t *testing.T) {
	...
	vm := &VM{}
	err := vm.Initialize(context.Background(), ctx, db, genesisBytes, []byte{}, []byte{}, nil, []*commonEng.Fx{}, &enginetest.Sender{})
	require.NoError(t, err)
	...
	ctx, _, _, _ = setupGenesis(t, genesisJSONLatest)
	err = vm.Initialize(context.Background(), ctx, db, genesisBytes, []byte{}, []byte(config), nil, []*commonEng.Fx{}, &enginetest.Sender{})
	require.NoError(t, err)
