...
func (b *blockBuilder) needToBuild() bool {
	...
	need := size > 0 || b.mempool.Len() > 0
	return need
}

// markBuilding adds a PendingTxs message to the stateSyncDone channel.
// markBuilding assumes the [buildBlockLock] is held.
func (b *blockBuilder) markBuilding() {
	b.pendingSignal.Broadcast() // <--- to add to subnet-EVM / coreth
...
func (b *blockBuilder) waitForPendingTxs(ctx context.Context, pending chan commonEng.Message) <-chan commonEng.Message {
	var wg sync.WaitGroup
	wg.Add(1)

	go func() {
		defer wg.Done()
		b.waitForPendingTx(ctx, pending)
	}()
	...
	case <-ctx.Done():
		b.buildBlockLock.Lock()
		defer b.buildBlockLock.Unlock()
		b.pendingSignal.Signal()
	}

	return pending
}

func (b *blockBuilder) waitForPendingTx(ctx context.Context, output chan<- commonEng.Message) {
	b.buildBlockLock.Lock()
	defer b.buildBlockLock.Unlock()
