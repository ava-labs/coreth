...
func TestAtomicTxGossip(t *testing.T) {
	...
	require.NoError(err)
	tx, err := atomic.NewImportTx(vm.ctx, vm.currentRules().AvalancheRules, vm.clock, vm.ctx.XChainID, address, initialBaseFee, secp256k1fx.NewKeychain(pk), []*avax.UTXO{utxo})
	require.NoError(err)
