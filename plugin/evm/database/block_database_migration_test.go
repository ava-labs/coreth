...
func TestBlockDatabaseMigrator_Status(t *testing.T) {
	t.Run("Migration status is not started after initialization", func(t *testing.T) {
		...
		require.Equal(t, migrationStatusNotStarted, migrator.status)
	})
	...
	t.Run("Migration status is in progress during migration", func(t *testing.T) {
		...
		writeKVDBBlocks(t, chaindb, blocks)
		...
		require.Eventually(t, func() bool {
			return migrator.status == migrationStatusInProgress
	...
	t.Run("Migration status is completed after migration", func(t *testing.T) {
		...
		require.Equal(t, migrationStatusCompleted, migrator.status)
	})
}
...
func TestWrappedBlockDatabase_Migration(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			migratordb := prefixdb.New(blockdbMigrationPrefix, base)
			if tc.initStatus != nil {
				require.NoError(t, migratordb.Put(migrationStatusKey, []byte{byte(*tc.initStatus)}))
			}
			...
			writeKVDBBlocks(t, chaindb, kvdbBlocks)
			...
			migratordb = prefixdb.New(blockdbMigrationPrefix, base)
			status, err := migratordb.Get(migrationStatusKey)
			require.NoError(t, err)
...
func writeKVDBBlocks(t *testing.T, chaindb ethdb.Database, blocks []*types.Block) {
	for _, block := range blocks {
