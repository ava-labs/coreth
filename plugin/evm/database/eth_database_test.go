...
func newDatabasesFromDir(t *testing.T, dataDir string) (*WrappedEthDatabase, ethdb.Database) {
...
func TestWrappedEthDatabase_Read(t *testing.T) {
	// Test that WrappedEthDatabase reads block data correctly
	dataDir := t.TempDir()
...
func TestWrappedEthDatabase_Write(t *testing.T) {
	dataDir := t.TempDir()
	wrapper, _ := newDatabasesFromDir(t, dataDir)
	blocks, receipts := createBlocks(t, 3)
	writeBlocks(wrapper, blocks[1:], receipts[1:])
}

func TestWrappedEthDatabase_Delete(t *testing.T) {
	// Test WrappedEthDatabase delete operations.
...
func TestWrappedEthDatabase_WriteOrder_CachesUntilBoth(t *testing.T) {
...
func TestWrappedEthDatabase_Batch(t *testing.T) {
...
func TestWrappedEthDatabase_SameBlockWrites(t *testing.T) {
...
func TestWrappedEthDatabase_DifferentBlocksSameHeight(t *testing.T) {
...
func TestWrappedEthDatabase_EmptyReceipts(t *testing.T) {
	...
	for _, block := range blocks {
		...
		_, err := wrapper.receiptsDB.ReadBlock(blockNum)
		require.NoError(t, err)
...
func TestWrappedEthDatabase_Close_PersistsData(t *testing.T) {
	...
	_, err := wrapper.blockDB.ReadBlock(block.NumberU64())
	require.Error(t, err)
...
func TestWrappedEthDatabase_ReadDuringMigration(t *testing.T) {
...
func TestWrappedEthDatabase_New_Initialization(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:                "existing_db_min_height",
			existingDBMinHeight: uint64Ptr(2),
			stateSyncEnabled:    false,
			chainDBBlocks:       blocks[5:8],
			expInitialized:      true,
			expMinHeight:        uint64Ptr(2),
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Add blocks to chainDB if needed
			...
			require.Equal(t, tc.expInitialized, wrapper.IsInitialized())
			if tc.expMinHeight != nil {
				require.NotNil(t, wrapper.minHeight)
				require.Equal(t, *tc.expMinHeight, *wrapper.minHeight)
			} else {
				require.Nil(t, wrapper.minHeight)
			}
...
func TestWrappedEthDatabase_New_AutoMigrate(t *testing.T) {
	blocks, _ := createBlocks(t, 10)

	// Test cases for auto-migration behavior
	testCases := []struct {
		name                   string
		stateSyncEnabled       bool
		chainDBBlocks          []*types.Block
		...
		expMinHeight           *uint64
		expBlocks              uint64
		expMigrationNotStarted bool
	}{
		{
			name:             "empty_database_no_state_sync",
			stateSyncEnabled: false,
			expMinHeight:     uint64Ptr(1),
		},
		{
			name:             "blocks_to_migrate",
			stateSyncEnabled: false,
			chainDBBlocks:    blocks[5:10],
			expMinHeight:     uint64Ptr(5),
			expBlocks:        5,
		...
		{
			name:             "blocks_to_migrate_disabled",
			stateSyncEnabled: false,
			chainDBBlocks:    blocks[5:10],
			expMinHeight:     uint64Ptr(5),
			expBlocks:        5,
		...
		{
			name:                   "no_blocks_state_sync",
			stateSyncEnabled:       true,
			expMinHeight:           nil, // not set yet
			expMigrationNotStarted: true,
		},
		{
			name:                "existing_db_no_migration",
			stateSyncEnabled:    false,
			chainDBBlocks:       blocks[5:10], // chain blocks will be ignored
			existingDBMinHeight: uint64Ptr(2),
			expMinHeight:        uint64Ptr(2),
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.existingDBMinHeight != nil {
				// force the database not to be initialized automatically by ensuring
				// we have no chainDB blocks and state sync is enabled
				wrapper, err := NewWrappedEthDatabase(
					base,
					blockDBPath,
					chainDB,
				...
				// initialize the database with the existing min height
				require.NoError(t, wrapper.InitWithMinHeight(*tc.existingDBMinHeight))

				// make sure the existing database is migrated
				require.NoError(t, wrapper.migrator.Migrate())
				waitForMigratorCompletion(t, wrapper.migrator, 10*time.Second)
			...
			// Add blocks to chainDB if needed
			for _, block := range tc.chainDBBlocks {
				writeBlocks(chainDB, []*types.Block{block}, []types.Receipts{})
			}

			// Create wrapper database with auto-migration
			wrapper, err := NewWrappedEthDatabase(
				base,
				blockDBPath,
				chainDB,
			...
			if tc.expMigrationNotStarted {
				require.Nil(t, wrapper.migrator)
				require.Zero(t, tc.expBlocks)
			} else {
				require.NotNil(t, wrapper.migrator)

				waitForMigratorCompletion(t, wrapper.migrator, 10*time.Second)
				blocksProcessed := wrapper.migrator.getBlocksProcessed()
				require.Equal(t, tc.expBlocks, blocksProcessed)
			}
...
func TestWrappedEthDatabase_Genesis(t *testing.T) {
	dataDir := t.TempDir()
	wrapper, chainDB := newDatabasesFromDir(t, dataDir)
