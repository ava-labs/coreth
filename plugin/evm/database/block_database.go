...
const (
	// blockCacheSize is the maximum number of blocks to cache
	blockCacheSize = 256
)
...
func NewWrappedBlockDatabase(db database.Database, blockdb database.BlockDatabase, chaindb ethdb.Database, withMigrator bool) (*wrappedBlockDatabase, error) {
	...
	return &wrappedBlockDatabase{
		...
		bodyCache:   sync.Map{},
		headerCache: sync.Map{},
		blockCache:  lru.NewCache[uint64, []byte](blockCacheSize),
		migrator:    migrator,
	}, nil
}
...
func (db *wrappedBlockDatabase) Put(key []byte, value []byte) error {
	...
	db.blockCache.Add(blockNumber, encodedBlock)
...
func (db *wrappedBlockDatabase) Has(key []byte) (bool, error) {
	...
	if encodedBlock, ok := db.blockCache.Get(blockNumber); ok {
		if data := extractBlockData(encodedBlock, blockHash, key); data != nil {
			return true, nil
		}
	}
