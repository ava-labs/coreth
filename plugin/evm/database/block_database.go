...
func (db *BlockDatabase) createMeteredBlockDatabase(namespace string, minHeight uint64) (database.HeightIndex, error) {
	...
	meteredDB, err := meterblockdb.New(db.reg, namespace, newDB)
	if err != nil {
...
func (db *BlockDatabase) getReceipts(key []byte, blockNumber uint64, blockHash common.Hash) ([]byte, error) {
	...
	if err != nil {
		if errors.Is(err, blockdb.ErrBlockNotFound) && db.isMigrationNeeded() {
			return db.Database.Get(key)
		}
		return nil, err
	}
...
func (db *BlockDatabase) shouldWriteToBlockDatabase(key []byte) bool {
	if !db.initialized {
		...
		log.Warn("shouldWriteToBlockDatabase called on a block database that is not initialized", "stack", string(debug.Stack()))
		return false
	}
