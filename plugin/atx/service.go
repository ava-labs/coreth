...
type AvaxAPI struct {
	VM              *VM
	VmVersion       string
	EstimateBaseFee func(context.Context) (*big.Int, error)
}
...
func (service *AvaxAPI) Version(r *http.Request, _ *struct{}, reply *VersionReply) error {
	reply.Version = service.VmVersion
	return nil
}
...
func (service *AvaxAPI) Import(_ *http.Request, args *ImportArgs, response *api.JSONTxID) error {
	...
	if args.BaseFee == nil {
		...
		baseFee, err = service.EstimateBaseFee(context.Background())
		if err != nil {
			return err
		}
	} else {
	...
	service.VM.AtomicTxPushGossiper.Add(&GossipAtomicTx{Tx: tx})
	return nil
}
