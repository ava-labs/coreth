...
TEMPORARY_EXCLUDES="--exclude-dir=vm --exclude=evm.go"
extra_imports=$(grep -r --include='*.go' --exclude-dir='geth' ${TEMPORARY_EXCLUDES} '"github.com/ethereum/go-ethereum/.*"' -o -h | sort -u | comm -23 - ./scripts/geth-allowed-packages.txt)
