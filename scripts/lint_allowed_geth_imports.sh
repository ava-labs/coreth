...
EXCLUDES='--exclude evm.go --exclude tx_access_list.go --exclude tracers.go'
extra_imports=$(grep -r --include='*.go' ${EXCLUDES} '"github.com/ethereum/go-ethereum/.*"' -o -h | sort -u | comm -23 - ./scripts/geth-allowed-packages.txt)
